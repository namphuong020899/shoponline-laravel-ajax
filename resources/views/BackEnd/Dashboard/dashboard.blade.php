         ...
         <div class="ibox ">
            <div class="ibox-title">
               ...
               <div class="float-right">
                  <div class="btn-group">
                     <select id="choose_select" name="choose_select" class="btn btn-xs btn-white dashboard-filter">
                        <option value="7ngay">7 Day</option>
                        <option value="thangtruoc">Last Month</option>
                        <option value="thangnay">This Month</option>
                        <option value="365ngay">One Year</option>
                     </select>
                  </div>
               </div>
            </div>
                  ...
                  <div class="col-lg-3">
                     <ul class="stat-list">
                        <li>
                           ...
                           <small>Total orders in sales</small>
                           <div class="stat-percent">{{round(($tn_sales-$ttr_sales)/$ttr_sales*100,0)/100}}% 
                            @if (round(($tn_sales-$ttr_sales)/$ttr_sales*100,0)/100 <= 0)
                              <i class="fa fa-level-down text-navy text-danger"></i>
                            @else
                              <i class="fa fa-level-up text-navy"></i>
                            @endif
                          </div>
                           <div class="progress progress-mini">
                              <div style="width: {{round(($tn_sales-$ttr_sales)/$ttr_sales*100,0)/100}}%;" class="progress-bar {{ round(($tn_sales-$ttr_sales)/$ttr_sales*100,0)/100 <= 0 ? 'progress-bar-danger' : ''  }}"></div>
                           </div>
                        </li>
                        <li>
                           <h2 class="no-margins ">{{ $tn_total }}</h2>
                           <small>Orders</small>
                           <div class="stat-percent">{{round(($tn_total-$ttr_total)/$ttr_total*100,0)/100}}%
                            @if (round(($tn_total-$ttr_total)/$ttr_total*100,0)/100 <= 0)
                              <i class="fa fa-level-down text-navy text-danger"></i>
                            @else
                              <i class="fa fa-level-up text-navy"></i>
                            @endif
                           ...
                           <div class="progress progress-mini">
                              <div style="width: {{round(($tn_total-$ttr_total)/$ttr_total*100,0)/100}}%;" class="progress-bar {{ round(($tn_total-$ttr_total)/$ttr_total*100,0)/100 <= 0 ? 'progress-bar-danger' : ''  }}"></div>
                           </div>
                        </li>
                        <li>
                           <h2 class="no-margins ">{{ number_format($tn_profit) }}</h2>
                           <small>Total orders in profit</small>
                           <div class="stat-percent">{{round(($tn_profit-$ttr_profit)/$ttr_profit*100,0)/100}}% 
                            @if (round(($tn_sales-$ttr_sales)/$ttr_sales*100,0)/100 <= 0)
                              <i class="fa fa-level-down text-navy text-danger"></i>
                            @else
                              <i class="fa fa-level-up text-navy"></i>
